...
resid = np.squeeze(model.predict(X_train)) - y_train

fig = plt.figure(figsize=(13,8))
plt.plot(resid, "b-")
skip_minutes = len(resid)//6
plt.xticks(np.arange(0, len(resid), skip_minutes), 
           df_train.index[LAG::skip_minutes])
plt.title("Fitted residuals of 6-8-8-1 LSTM", fontsize=20)
plt.ylabel("")
...
print(acorr_ljungbox(resid, lags=[100], model_df=LAG))
print(acorr_ljungbox(resid**2, lags=[100], model_df=LAG))
...
skip_minute = len(y_close_test)//10
plt.xticks(np.arange(0, len(y_close_test), skip_minute), 
           xticks[::skip_minute])
