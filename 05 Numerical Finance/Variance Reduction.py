...
for i in range(1000):
    u = np.random.rand(n)
    x, z = u**3 * (1-u)**2.5, (1-u)**2.5
    # Using sample variance and sample covariance
    s_cov = np.cov(x, z, ddof=1)[0][1]
    s_var = np.var(z, ddof=1)
    psi_C.append(np.mean(x) - s_cov/s_var * (np.mean(z) - mu_z))
...
n, psi_MC = 10000, []
mu_z = np.array([2/7, 1/4])
for i in range(1000):
    u = np.random.rand(n)
    x, z = u**3 * (1-u)**2.5, np.vstack([(1-u)**2.5, u**3])
    # Using sample variance and sample covariance
    s_cov = np.cov(x, z, ddof=1)[0,1:]
    diff_z = np.mean(z, axis=1) - mu_z
    inv_s_var = np.linalg.inv(np.cov(z, ddof=1))
    psi_MC.append(np.mean(x) - s_cov @ inv_s_var @ diff_z)
