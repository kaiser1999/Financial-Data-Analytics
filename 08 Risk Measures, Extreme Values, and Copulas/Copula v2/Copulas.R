...
# residuals with respect to 45 degree line
resid2_N <- (sort(returns_md2) - q_N)^2
resid2_t <- (sort(returns_md2) - q_t)^2
(idx_start <- min(which(q_N > 15), which(q_t > 15)))
plot(sort(resid2_N)[idx_start:n_days], 
     ylab="squared residuals", pch=1, cex=1.5)
points(sort(resid2_t)[idx_start:n_days], 
       pch=4, cex=1.5, lwd=2)
