...
resid2_N = (np.sort(returns_md2) - q_N)**2
resid2_t = (np.sort(returns_md2) - q_t)**2

fig = plt.figure(figsize=(10, 10), dpi=200)
# plot theoretical quantiles starting from 10
idx_start = min(min(np.where(q_N>15)[0]), min(np.where(q_t>15)[0]))
print(idx_start)
plt.plot(np.sort(resid2_N[idx_start:]), "ro", markersize=10)
plt.plot(np.sort(resid2_t[idx_start:]), "bx", markersize=10)
